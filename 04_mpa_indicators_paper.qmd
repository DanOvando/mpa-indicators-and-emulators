...
prep_run(run_name = "indicators_v0.3",figure_text_size = 11) # loads packages and creates and returns some global variables for the analysis
...
state_experiments <- baseline_state_experiments %>%
  ...
  mutate(
    ...
    adult_home_range = sample(c(1, 5, 20), length(state_id), replace = TRUE),
    recruit_home_range = sample(c(1, 10, 100), length(state_id), replace = TRUE),
...
fig <- mpa_outcomes |>
 ...
 # figsaver(fig)
  ...
  # stat_interval(aes(difficulty, value)) +
  # geom_density(aes(value, after_stat(scaled),fill = difficulty, color = difficulty),alpha = 0.25) +
  facet_wrap(~mpa_bin, scales = "free_y") +
  scale_x_discrete(name = '', labels = c("Biomass Inside", "Biomass Outside", "Total Biomass", "Catch") ) +
  scale_y_continuous(name= "% Change",limits = c(NA,2), oob = squish, breaks = sighbreaks, labels = sighlabs) +
  theme(legend.position = "bottom",
