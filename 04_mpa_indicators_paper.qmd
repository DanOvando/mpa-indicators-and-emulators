...
prep_run(run_name = "indicators_v0.52",figure_text_size = 11, rx = 20, ry = 20, patch_area = 5^2) # loads packages and creates and returns some global variables for the analysis
...
To answer the first question (*How variable can MPA effects Be?*), we
use a spatially-explicit multi-species and multi-fleet bio-economic
model [@ovando2023a] to simulate the effect of no-take MPAs on biomass
and catch across a wide range of possible states of the underlying
...
3.  Simulate the system forward in time with a desired number of patches
    protected in a no-take MPA.

4.  Calculate indicators of MPA performance based on the with-MPA
    scenario.

5.  Calculate true outcomes of the MPA by comparing values (e.g. total
    biomass and catch) per species and per fleet in the simulation with
    MPAs versus the paired simulation without MPAs and converting into a
    percentage effect of the MPA on the outcome in question.

6.  Join true MPA outcomes to estimated indicator values and compare.

#### Simulating MPA Effects

We modeled four different species atchetypes in this paper intended to
reflect a range of life histories. These simulated species are based on
a tuna (*Thunnus albacares*), shark (*Carcharhinus falciformis*),
grouper (*Epinephelus fuscoguttatus*), and reef fish (*Seriola
quinqueradiata*) *.* We created a fixed template for each of these
species designed to capture core attributes such as growth, mortality,
and movement.Core growth, maturity, and mortality parameters were fixed
based on available literature for these species (see SI for details).
The steepness *h* of the Beverton-Holt stock-recruitment [@mace1988]
controls the rate at which recruitment changes as a function of spawning
stock biomass, and is generally poorly understood. We fixed steepness to
reflect a range of highly sensitive spawner-recruit relationship (silky
sharks *h=0.3,* yellowfin tuna *h = 0.9*, grouper *h = 0.6,* reef fish
*h = 0.8*). We set movement rates of recruits and post-recruit fish
based on a home range parameter, set such that after one year 95% of
animals were within a given linear distance of their point of origin at
the start of the year. For yellowfin tuna we set this value at 250KM for
recruits and post-recruits (high dispersal at all life stages), 250KM
and 2.5KM for silky sharks (high adult dispersal, low recruit
dispersal), low and medium for grouper, and medium and medium for reef
fish. Remaining simulation bio-economic parameters beyond these core
...
[@lester2009] is modeled using *Biomass* as the dependent variable,
...
alternative distributions as appropriate. For the main results presented
in this paper, all models presented in @tbl-indicators are fit at the
level of individual species (or individual species and fleets when the
dependent variable is catch). Our results were insensitive to using the
total change in biomass or catch across all species (see Fig.SXX).

For *Inside vs. Outside* comparisons, the selection of the *Outside*
site is not always straightforward. Comparing biomass densities inside
an MPA on a rocky reef to biomass densities on the sandy flats outside
...
*Near* and *Far* categories. We followed the general pattern of
...
|------------|------------|------------|------------|------------|------------|
| Biomass Density Before-After -Control-Impact | Inside vs. Outside | $log(B)\sim\beta_0 + \beta_1INSIDE + \beta_2AFTER +\pmb{\beta_3}INSIDE \times AFTER$ | Ratio of change in B after MPA implementation inside MPA relative to outside MPA | Parallel trends in biomass density inside and outside MPA pre- and post-implementation. | @lester2009 |
| Biomass Density Response Ratio | Inside vs. Outside | $log(B) \sim \beta_0 + \pmb{\beta_1}INSIDE$ | Post-MPA Ratio of biomass inside MPA relative to outside MPA | No pre-existing differences in *B* between inside and outside sites | @smith2024b |
| Mean Length Response Ratio | Inside vs. Outside | $log(ML) \sim \beta_0 + \pmb{\beta_1}INSIDE$ | Post-MPA ratio of mean length inside MPA relative to outside MPA | No pre-existing differences in *ML* between inside and outside sites | @halpern2003 |
| Effort Gradient | Near vs. Far | $log(E) \sim \beta_0 + \pmb{\beta_1}NEAR$ | Post-MPA ratio of fishing effort NEAR MPA relative to FAR | No pre-existing differences in *E* between NEAR and FAR sites | @goni2008 |
| Biomass Density Gradient | Near vs. Far | $log(B) \sim \beta_0 + \pmb{\beta_1}NEAR$ | Post-MPA ratio of biomass density NEAR MPA relative to FAR | No pre-existing differences in *B* between NEAR and FAR sites | @halpern2009 |
| Biomass Density Before-After Gradient | Near vs. Far | $log(B)\sim\beta_0 + \beta_1NEAR + \beta_2AFTER +\pmb{\beta_3}NEAR \times AFTER$ | Ratio of change in B after MPA implementation near to MPA relative to far from MPA |  | @methratta2020 @lynham2024 @medoff2022 |
...
The preceding sections describe how we simulate the true effect of the
MPAs on a range of outcomes, and how we calculate empirical indicators
from the simulated data. We created factorial comparisons of these two
results, creating a pair of every outcome with every indicator for each
of our simulated states. We used several methods to measure the ability
of each indicator to reflect each MPA outcome.

We measured the correlation between each indicator and each outcome
using Spearman rank correlations ($\rho$), given the potential for
highly different scales between the indicator and the outcomes, the
potential for non-linear relationships between the indicator and the
outcomes. We converted these ρ values into psuedo-R^2^ values (ρ^2^)
that reflect the proportion of the rank-order variance in the outcome is
explained by the indicator. These Spearman rank correlation values
provide a general sense of the extent to which a higher indicator value
at one MPA and a lower indicator value at another MPA corresponds to a
higher outcome value at the first MPA and a lower outcome value at the
second MPA.
...
state_experiments <- baseline_state_experiments %>%
  ...
  mutate(
    ...
    f_v_m = runif(length(state_id), 0.025, 0.24),
    hyperallometry = sample(c(1, 2), length(state_id), replace = TRUE),
    sigma_rec = sample(c(0.33), length(state_id), replace = TRUE),
...
delta <- fauna_experiment |> 
 ...
 scale_linetype(name = "") + 
  ...
  scale_y_continuous(name = "Biomass",limits = c(0, NA), expand = c(0, NA)) +
...
all measured outcomes (@fig-outcomes).

Increasing MPA size generally acted to increase the range of possible
effects. MPAs almost always (`r better_inside` of simulations) caused an
increase in *Biomass Inside* their borders, though negative effects were
also possible. Simulated MPA increased *Biomass Outside* their borders
in `r better_outside` of simulations, and resulted in a net decrease in
*Biomass Outside* their borders in `r worse_outside` of simulations. The
balance between biomass most frequently increasing inside and decreasing
outside resulted in MPAs resulting in a net increase in *Total Biomass*
in `r better_total` of simulations and a net decrease in `r worse_total`
of simulations. Negative biomass outcomes for *Biomass Inside* and
*Total Biomass* were much more common in scenarios where the MPA
placement strategy was to avoid fishing, as these scenarios generally
resulted in the concentration of fishing effort in prime habitat. The
possibility of unintended losses in biomass increased with the level of
...
fig <- mpa_outcomes |>
  ...
  scale_color_viridis_d(name = "B/B0") +
...
indicators were positively correlated with *Biomass Inside* and *Total
Biomass*, with *Biomass Density BACI* having the highest $\rho$ values
for both ($\rho^2 = 0.72$ and $\rho^2 = 0.41$ respectively), followed by
*Biomass Density Response Ratio* ($\rho^2 = 0.57$ and $\rho^2 = 0.31$
respectively). All three *Inside vs. Outside* indicators were negatively
correlated with *Biomass Outside*, meaning that simulations with
relatively higher response ratios were associated with simulations with
more negative *Biomass Outside* outcomes. Gradient based indicators had
much lower correlations across all outcomes (all $|\rho| < 0.3$), and
explained almost none of the rank-level variation in outcomes. *Inside
vs. Outside* indicators were somewhat negatively correlated with catch
outcomes, but weakly so. In summary, *Inside vs. Outside* indicators had
relatively strong but variable correlations with *Biomass* outcomes,
*Gradient* based indicators explained almost none of the rank-level
variation in any of the outcomes, and none of the indicators explained
any meaningful amount in the rank-level variation in *Catch*
(@fig-rho-error-bias A).

Turning to the RMSE and Bias results results, interpreting the estimated
percentage effects produced by the indicators as directly indicative of
the true percentage effect in general produced relatively high levels of
error and bias. *Biomass Density BACI* values were slightly positively
biased with regards to *Biomass Inside*, and had a RMSE of 44 percentage
points. *Biomass Density Response Ratios* had similar levels of error
and bias to BACI. Interpreted literally, both of these indicators would
overestimate the true effects on all of the metrics, often by 30 or more
percentage points, with RMSE values of 50-100 percentage points. XX this
section is rough.

```{r}
#| label: fig-rho-error-bias
#| fig-cap: "A) Spearman correlaions $\\rho$ (color) and  $\\rho^2$ (text) between indicators (y-axis) and outcomes (x-axis) at the level of individual species and fleets across all simulations. B) Root mean squared error (RMSE, text) and mean error (Bias, color) between indicators (y-axis) and outcomes (x-axis) at the level of individual species and fleets across all simulations. "
#| fig-height: 7
...
@fig-rho-error-bias. Simulated response ratios was strongly positivly
correlated with *Biomass Inside* ($\rho = 0.82$), but were much less
correlated for the other variables, least of all *Catch.* However, these
results do present a visual illustration of the potential negative
correlation between higher response ratios and *Biomass Outside* and
*Catch* (@fig-rr-performance).

```{r}
#| label: fig-rr-performance
#| fig-cap: "Simulated Biomass Density Response Ratios (x-axis) plotted against simulated MPA effects (y-axis), with distribution of response ratios matched to empirical response ratios of targeted finfish reported by @smith2024b."
...
XX Future research could dig into the mechanissm behind different
outcomes to see if indicators could be identified to narrow scope of
outcomes

This study sought to answer two questions: How variable can the effects
of MPAs be on a range of outcomes, and how well do common empirical
indicators measure these effects? Our results show that while MPAs have
some clear common sense outcomes, namely almost always increasing
biomass inside their borders, the magnitude and direction of their
effects on biomass and catch can vary substantially. We also show that
some indicator performance varied widely depending on the indicator and
outcome in question. We provide greater context for these results
before.
...
fish inside that area. Exactly by how much can vary greatly depending on
how heavy fishing pressure would have been without the MPA, the size of
the MPA relative to the movement of the species, and the placement of
the MPA. However, under our simulation framework that allows for
heterogeneous habitats and fleet dynamics, these increases in biomass
inside often came at a cost in net decreases in biomass outside; in the
majority of our simulations (`r worse_outside`), total biomass went
down, not up, outside the MPA. This is not because spillover was not
occurring, but because the magnitude of spillover from increases inside
the MPA was not enough overcome the concentration of fishing effort
outside. The magnitude of increases inside were generally sufficiently
greater than the losses outside that in most simualtions
...
This unintended consequence of biomass losses outside or in total occurs
in our model due to fleet dynamics. MPA can reduce biomass outside their
borders by concentrating effort in the remaining fishing grounds to a
level that cannot be compensated for by spillover from the MPA. Beyond
this, if an MPA results in displacement of a fishing fleet from a
relatively low biomass area for a given species to a high biomass area
for that species, the net result can be net increase in the total
...
fleets. @abbott2012 provides a real world example of this phenomenon, as
does ... XX
...
empirical results showing negative outcomes as impossible should they
arise.

The question of whether spillover occurs we would argue is in fact not
really an open or particularly relevant question anymore; for almost any
reasonable case the movement dynamics of marine organiams are such some
level of spillover will almost always occur at one or more life stages.
The relevant test then is not "is spillover significantly different than
0", but "what is the net effect size of spillover".
...
That MPAs can have negative effects on catch is more intuitive, as the
...
negative at some point MPA size. Our results confirm that win-win
outcomes where MPAs cause a net increase in total biomass and total
catch are absolutely possible. There were also cases where MPAs did not
substantially harm catches but did result in increase biomass. However,
in the majority of our simulations MPAs resulted in a net decrease in
catch. XX

The search for gradients in MPAs has a long history.. Intuitive, since
it is the most accesible of data types, since it can be calculated using
only post-implementation data, and can be based on fishery-dependent
data outside the reserve rather than paying for fishery-independent data
inside the reserve where fishing is no longer allowed

Together then, our results show that the effects of MPAs on biomass and
catch can be highly variable and context dependent. Given this ...
