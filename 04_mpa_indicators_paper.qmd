...
"It uses the means of biomass (for targeted and nontargeted fish species)... inside and outside of SMRs across the 2019-20 sampling period as a measure of effect size – that is, an MPA effect."
...
1. determine best individaul predictor and for each metric plot the relationship of the best predictor

2. Somewhere rank all indicators just so they don't get lost in the discourse

2. Performance of "ensemble" predictor 
...
indicator_performance |> 
  ...
  ggplot(aes(reorder(indicator, r.squared), r.squared)) + 
...
indicator_performance |> 
  filter(name %in% c("biomass", "mpa_biomass", "catch")) |> 
  ggplot(aes(reorder(indicator, -rmse), rmse)) + 
  stat_gradientinterval() + 
  coord_flip() + 
  facet_wrap(~name)
...
quadrant_outcomes$quadrant

predictors <- quadrant_outcomes |> 
  select(quadrant, ends_with("raw")) |> 
  na.omit() |> 
  mutate(quadrant = as.factor(quadrant))

quadrant_model <- ranger(quadrant ~ ., predictors, classification = TRUE)


confusion_matrix <- (quadrant_model$confusion.matrix) |> 
  as.data.frame()

confusion_matrix |> 
  tibble() |> 
  group_by(predicted) |> 
 mutate(pn = Freq / sum(Freq),
