...
prep_run(run_name = "indicators_v0.5",figure_text_size = 11, rx = 20, ry = 20, patch_area = 5^2) # loads packages and creates and returns some global variables for the analysis
...
structure for any one simulation run is as follows:
...
| Indicator | Equation | Estimated Effect | Key Assumptions\* | Example Citations |
|---------------|---------------|---------------|---------------|---------------|
| Biomass Density Before-After -Control-Impact | $log(B)\sim\beta_0 + \beta_1INSIDE + \beta_2AFTER +\pmb{\beta_3}INSIDE \times AFTER$ | Ratio of change in B after MPA implementation inside MPA relative to outside MPA | Parallel trends in biomass density inside and outside MPA pre- and post-implementation. | @lester2009 |
| Biomass Density Response Ratio | $log(B) \sim \beta_0 + \pmb{\beta_1}INSIDE$ | Post-MPA Ratio of biomass inside MPA relative to outside MPA | No pre-existing differences in *B* between inside and outside sites | @smith2024b |
| Mean Length Response Ratio | $log(ML) \sim \beta_0 + \pmb{\beta_1}INSIDE$ | Post-MPA ratio of mean length inside MPA relative to outside MPA | No pre-existing differences in *ML* between inside and outside sites | @halpern2003 |
| Effort Gradient | $log(E) \sim \beta_0 + \pmb{\beta_1}NEAR$ | Post-MPA ratio of fishing effort NEAR MPA relative to FAR | No pre-existing differences in *E* between NEAR and FAR sites | @goni2008 |
| Biomass Density Gradient | $log(B) \sim \beta_0 + \pmb{\beta_1}NEAR$ | Post-MPA ratio of biomass density NEAR MPA relative to FAR | No pre-existing differences in *B* between NEAR and FAR sites | @halpern2009 |
| Biomass Density Before-After Gradient | $log(B)\sim\beta_0 + \beta_1NEAR + \beta_2AFTER +\pmb{\beta_3}NEAR \times AFTER$ | Ratio of change in B after MPA implementation near to MPA relative to far from MPA |  | @methratta2020 @lynham2024 @medoff2022 |
...
XX Future research could dig into the mechanissm behind different outcomes to see if indicators could be identified to narrow scope of outcomes
