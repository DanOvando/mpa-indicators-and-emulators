...
We modeled four different species in this paper intended to reflect a
range of life histories. These simulated species are based on Yellowfin
tuna (*Thunnus albacares*), silky shark (*Carcharhinus falciformis*),
goliath grouper (*Epinephelus fuscoguttatus*), and *XX .* We created a
fixed template for each of these species designed to capture core
attributes such as growth, mortality, and movement.Core growth,
maturity, and mortality parameters were fixed based on available
literature for these species (see SI for details). The steepness *hI* of
the Beverton-Holt stock-recruitment [@mace1988] controls the rate at
which recruitment changes as a function of spawning stock biomass, and
is generally poorly understood. We fixed steepness to reflect a range of
highly sensitive spawner-recruit relationship (silky sharks *h=0.3,*
yellowfin tuna *h = 0.9*, grouper *h = 0.6,* XX *h = 0.8*). We set
movement rates of recruits and post-recruit fish based on a home range
parameter, set such that after one year 95% of animals were within a
given linear distance of their point of origin at the start of the year.
For yellowfin tuna we set this value at 250KM for recruits and
post-recruits (high dispersal at all life stages), 250KM and 2.5KM for
silky sharks (high adult dispersal, low recruit dispersal), low and
medium for grouper, and medium and medium for reef fish. Remaining
simulation bio-economic parameters beyond these core template values
were randomly selected for each simulation, see @tbl-states.
...
| Recruitment Variation | XX |
| Recruitment autocorrelation | XX |
| Cross-Species Recruitment Correlations | XX |
...
| Biomass Density Response Ratio | $log(B) \sim \beta_0 + \pmb{\beta_1}INSIDE$ | Post-MPA Ratio of biomass inside MPA relative to outside MPA | No pre-existing differences in *B* between inside and outside sites | @lester2009 |
...
points, not percent changes. So, an bias value of 20 would mean that the
indicator is on average 20 percentage points higher than the true value,
not 20% higher.
