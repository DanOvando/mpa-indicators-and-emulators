...
prep_run(run_name = "indicators_test",figure_text_size = 8) # loads packages and creates and returns some global variables for the analysis
...
state_experiments$fauna[[1]][[1]]$b0
state_experiments$fauna[[1]][[2]]$b0
state_experiments$fauna[[1]][[3]]$b0
...
Example below shows how the MPA experiment works. Percent difference
between the two lines is the MPA effect.
...
fig <- ((alpha | beta) + plot_layout(guides = "collect") & theme(legend.position = "bottom"))
...
Another case study figure. The above hold MPA constant, this shows
change as a function of MPA size. Will decide which is more helpful.
...
This section of results explore how variable the simulated effects of
MPAs are.

Key results are that positive biomass more possible than negative but
both possible, negative and positive catch effects both possible, and
magnitude of effects in all directions increases with complexity
...
mpa_outcomes |>
  filter(observation_error == min(observation_error)) |>
  select(depletion,
         numeric_prop_mpa,
         difficulty,
         fished_biomass,
  ...
  facet_grid(mpa_bin ~ name, scales = "free_x", labeller = labeller(name = outcome_labeller)) + 
  scale_x_continuous(limits = c(NA,1), name = "% Change", labels = scales::percent) + 
...
mpa_outcomes |>
  ...
  geom_vline(xintercept = 0, linetype = 2) +
  stat_interval(aes(color_ramp = after_stat(level),color = difficulty)) +
...
Can also examine how frequency of simulated quadrant outcomes changes as
a function of MPA size, being clear throughout that cannot assign
probability of any one of these states of nature so can't say this means
that this distribution is representative of the real world per say.
...
We mapped each indicator value onto each MPA outcome, and then fit a
linear model to them and calculated which indicators were the "best
performer", as measured by RMSE, in being able to predict the outcome.
...
rrs <- best_performers_noncausal |>
  filter(indicator == "ind_biomass_rr",
         name %in% c("mpa_biomass","biomass", "catch"),
...
ca_rr_ind_plot <- sigh |>
  slice_sample(n = 1000) |> 
  mutate(name = fct_relevel(name, "mpa_biomass", "biomass")) |> 
  ggplot(aes(indicator_value, percent_mpa_effect)) +
...
tuned_fit_f <- parsnip::fit(tuned_model_workflow_f, training_split)

tuned_fit_dep <- parsnip::fit(tuned_model_workflow_dep, training_split)

tuned_fit_fonly <- parsnip::fit(tuned_model_workflow_fonly, training_split)
