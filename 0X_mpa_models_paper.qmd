...
```{r}
#|label: fig-max-catch
#| fig-cap: "Maximum potential percent increase in catch caused by an MPA predicted by the Highres (x-axis) and Lowres (y-axis) models."
compare_max_catch_mpa |> 
  ggplot(aes(highres_catch_percent,lowres_catch_percent)) + 
  geom_bin_2d(bins = 15) + 
  geom_abline(slope = 1, intercept = 0) + 
  facet_wrap(~difficulty) + 
  scale_x_continuous(limits = c(NA, 2), oob = squish, labels = scales::percent, name = "Highres Maximum Potential Catch Increase") +
  scale_y_continuous(limits = c(NA, 2), oob = squish,labels = scales::percent, name = "Lowres Maximum Potential Catch Increase") + 
  scale_fill_viridis_b(name = "# of Simulations")
...
highres_vs_lowres |> 
  ...
  ggplot(aes(percent_mpa_effect, lowres_percent_mpa_effect, color = depletion)) + 
  ...
  scale_x_continuous(
    name = "Highres % MPA Effect",
    labels = scales::percent,
    limits = c(NA, 2),
    oob = squish
  ) +
  scale_y_continuous(
    name = "Lowres % MPA Effect",
    labels = scales::percent,
    limits = c(NA, 2),
    oob = squish
  ) +
  scale_color_viridis_b(name = "BAU B/B0", option = "plasma") + 
...
xx make rank plot plotting the biomass and catch rank of each 
...
highres_vs_lowres |>
  ...
  ggplot(aes(highres_rank, lowres_rank)) + 
  ...
  scale_color_viridis_b(name = "BAU B/B0", option = "plasma", direction = -1) + 
  theme(legend.position = "top")
...
confusion_matrix <- lowres_quadrants |> 
  left_join(highres_quadrants) |> 
  filter(between(numeric_prop_mpa,0.2,0.4), kiss) |> 
  ...
  group_by(difficulty, highres_quadrant) |>
  mutate(pn = n / sum(n),
         label_pn = percent(pn, accuracy = 1)) 

confusion_matrix_plot <-  confusion_matrix |>
  ggplot(aes(highres_quadrant, lowres_quadrant)) +
  ...
  scale_x_discrete(name = "Highres Predicted Quadrant") +
  scale_y_discrete(name = "Lowres Predicted Quadrant")

agreement_plot <- confusion_matrix |> 
  filter(highres_quadrant == lowres_quadrant) |> 
  group_by(difficulty) |> 
  complete(highres_quadrant, fill = list(pn = 0)) |>
...
highres_vs_lowres |>
  ...
  ggplot(aes(prop_mpa,lowres_percent_mpa_effect - percent_mpa_effect , group = prop_mpa)) +
  geom_hline(yintercept = 0) +
...
pdp_fit  <- model_profile(
  explain_fit,
  variables = c("depletion", "steepness", "ssb0", "adult_diffusion", "habitat_patchiness", "range_cor"),
  type = "partial"
)
