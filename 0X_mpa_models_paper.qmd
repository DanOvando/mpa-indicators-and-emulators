...
prep_run(run_name = "test") # loads packages and creates and returns some global variables for the analysis
...
ggplot() +
  geom_line(
    ...
    aes(prop_mpa, value / ssb0, color = placement_strategy)
  ) +
  geom_point(data = emulation, aes(prop_mpa, depletion_mpa, color = "Emulation")) +
  scale_y_continuous(limits = c(0, NA), name = "depletion")

ggplot() +
  geom_line(data = reality |> filter(name == "catch"),
            aes(prop_mpa, value, color = placement_strategy)) +
  geom_point(data = emulation, aes(prop_mpa, yield_mpa, color = "Emulation")) +
...
lowres_thirty_protected_plot <- plot_stuff |>
  ...
  ggplot(aes(100 * biomass_effect, 100 * yield_effect, color = rough_f)) +
  ...
  scale_x_continuous(name = "% Change in Species Biomass", limits = c(-50,NA), oob = squish) +
...
marlin_vs_pt_plot <- comparison |>
  ...
  ggplot(aes(
    pmin(100000, percent_mpa_effect),
    pmin(100000, emulated_percent_mpa_effect)
  )) +
  ...
  scale_x_continuous(
    "Highres Model MPA Effect",
    oob = squish,
    limits = c(-1,200)
  ) +
  scale_y_continuous("Lowres Model MPA Effect", limits = c(-1,200),
                     oob = squish) +
