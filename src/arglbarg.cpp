...
// [[Rcpp::export]]
List sim_mpa(double r, double k, double m, double u, double p_mpa,
  ...
  // what the fuck
  NumericVector yield(years,0.0);
  
  NumericVector hmpa(years,0.0);
  
  NumericVector inside_b(years,0.0);
  
  double last_inside_b;
  
  NumericVector outside_b(years,0.0);
  
  double last_outside_b;
  
  double growth_mult = 1.0;
  
  NumericVector tmp(years,0.0);
  ...
  for (int t = 0; t < years; t++){
    ...
    if (pt == 0){
      ...
      if (local_dd == 1){
        ...
        last_inside_b = inside_b[t+1];
        
        last_outside_b = outside_b[t+1];
    ...
    yield = u * outside_b[t+1];
    
  } // close years loop

  List out = List::create(
    Named("inside_b") = inside_b,
    _("outside_b") = outside_b,
    _("yield") = yield,
    _("growth_mult") = growth_mult
  );
