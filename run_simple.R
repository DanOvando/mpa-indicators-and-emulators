#### runs simple MPA indicator and emulator examples, where simple means one species and one fleet ####
...
prep_run() # loads packages and creates and returns some global variables for the analysis
...
state_experiments <- state_experiments %>%
  ungroup() %>%
  mutate(use_ports = sample(c(FALSE), nrow(.), replace = TRUE)) %>%
  mutate(
    fleet = pmap(
      list(
        fauna = fauna,
...
for (i in 1:nrow(state_experiments)) {
  ...
  ggsave(file.path(
    fig_dir,
    glue::glue("simple_state_{i}_habitat.pdf")
  ), tmp)
  
  ggsave(file.path(
    fig_dir,
    glue::glue("simple_state_{i}_fleet_allocation.pdf")
  ), tmp2)
...
emulated_experiment_results <-
  ...
  mutate(mpa_experiment = pmap(
    ...
    run_twopbd_mpas,
    .progress = "running simple emulator experiments"
  ))

write_rds(emulated_experiment_results,
          file = file.path(results_dir, "simple_emulated_experiment_results.rds"))
...
write_rds(placement_experiments,
          file = file.path(results_dir, "simple_placement_experiments.rds"))

write_rds(state_experiments,
          file = file.path(results_dir, "simple_state_experiments.rds"))

write_rds(emulated_state_experiments,
          file = file.path(results_dir, "simple_emulated_state_experiments.rds"))
...
pb <- progress_bar$new(
  format = "  Running simple MPA Experiments [:bar] :percent eta: :eta",
...
write_rds(experiment_results,
          file = file.path(results_dir, "simple_experiment_results.rds"))

processed_sims <- process_sims(difficulty_level = "simple", results_dir = results_dir)


test <- processed_sims$mpa_outcomes |>
...
thirty_protected_plot <- test |>
  filter(between(prop_mpa, 0.2, 0.4), placement_strategy != "avoid_fishing") |>
