...
critters <-
  tibble(
    scientific_name = difficulty_species[[difficulty]]
  )
...
state_experiments <- state_experiments %>%
  mutate(tmp = map2(fauna, fleet, init_condit, .progress = glue::glue("simulating initial {difficulty} conditions")))
...
emulated_state_experiments <- state_experiments |> 
  mutate(twopbd_params = map2(fauna, fleet, safely(fit_twopbd), .progress = glue("fitting {difficulty} emulators"))) |> 
...
write_rds(emulated_experiment_results,
          file = file.path(results_dir, glue("{difficulty}_emulated_experiment_results.rds")))
...
write_rds(placement_experiments,
          file = file.path(results_dir, glue("{difficulty}_placement_experiments.rds")))
...
write_rds(experiment_results,
          file = file.path(results_dir, glue("{difficulty}_experiment_results.rds")))

processed_sims <- process_sims(difficulty_level = difficulty, results_dir = results_dir)
